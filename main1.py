...
param = {'error_username': "", 'error_password': "", 'error_verify': "", 'error_email': ""}

def build_page(errors):

    username ="<label> Username: </label>"
    username_input = "<input type='text' name='username' value = ''/>" + errors['error_username']
    ...
    email_input = "<input type='text' name='email'/>" + errors['error_email']
...
class Welcome(MainHandler):
    def get(self):
        username_submitted = str(self.request.get("username"))
        self.response.write('Welcome, %(name)s' %{'name': username_submitted})
